...
GridFSStorage.prototype._store = function _store(req, file, cb) {
  ...
  const readStreamDecorator = {
    get: ()=> readStream
  };
  
  this._generate(req, file, readStreamDecorator)
    .then((fileSettings) => {
      let settings;
    ...
    .then((streamOpts) => {
      ...
      readStreamDecorator.get().pipe(writeStream);
    })
    .catch(cb);
};
 ...
 **/
GridFSStorage.prototype._generate = function _generate(req, file, readStreamDecorator) {
  ...
  try {
    if (isGeneratorFn(this._file)) {
      generator = this._file(req, file, readStreamDecorator);
      this._file = generator;
      result = generator.next();
      return this._handleResult(result, true);
    } else if (isGenerator(this._file)) {
      generator = this._file;
      result = generator.next([req, file, readStreamDecorator]);
      return this._handleResult(result, true);
    } else {
      result = this._file(req, file, readStreamDecorator);
